...
\chapter{Magical Rules}

\section{Magical Technology}\index{Magical Rules!Magical Technology}
...
\subsection{Magic Plus Technology}\index{Magical Rules!Magival Technology!Magic plus Technology}
...
\subsubsection{Frozen Timepiece}\index{Magical Rules!Magical Technology!Magic plus Technology!Frozen Timepiece}
...
\subsubsection{Truth Binoculars}\index{Magical Rules!Magical Technology!Magic plus Technology!Truth Binoculars}

\textbf{Level}: 1d6 + 2

\textbf{Form}: Pair of binoculars with a large runic symbol on them

\textbf{Effect}: Not only do these make it easy to see things far away, but looking through them also allows the viewer to see through illusions and see things that are normally invisible, assuming the effect has a level lower than that of the binoculars.

\textbf{Depletion}: 1–2 in 1d100 (check each use)
...
\subsection{Technology that Interacts with Magic}\index{Magical Rules!Magical Technology!Technology that Interacts with Magic}
...
\textbf{Effect}: Magically discovers if anything is watching or listening to the user right at that moment, and reveals the source. Electronic surveillance devices, long-range scopes, hidden cameras, and magical scrying attempts all trigger this effect. In all these cases, the “source” is the nearest representation. So a hidden microphone is revealed, but not the location of the listener. 

POWER DEVICE 

\textbf{Level}: 1d6 + 2 

\textbf{Effect}: Magically powers one device that can fit within an area a short distance across. The device is now fully powered, charged, or fueled. If the cypher is used on an automobile, for example, the gas tank is full. If used on a flashlight, the battery is fully charged. 

SCREEN CONTROL 

\textbf{Level}: 1d6 + 2 

\textbf{Effect}: A technological screen (a television, computer monitor, smartphone, or the like) within short range shows whatever the user wishes for up to one minute per cypher level. The display can be pictures, text, or meaningless shapes and colors.
...
\section{Mind Control}\index{Magical Rules!Mind Control}
...
\textbf{A rule for any game}: don’t use mind control (or anything) to make a character have sex without the player’s permission. For more information and guidelines about consent in RPGs, read the free Consent in Gaming PDF at myMCG.info/consent

\section{Mystical Martial Arts}\index{Magical Rules!Mystical Martial Arts}
...
\section{Possession}\index{Magical Rules!Possession}
...
\section{Secret and True Names}\index{Magical Rules!Secret and True Names}
...
\section{Wishes}\index{Magical Rules!Wishes}
