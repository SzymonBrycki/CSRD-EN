...
\chapter{Optional Rules for Horror}\index{Horror Rules!Optional Rules}

This chapter describes many different optional rules (called “horror modules”) for making horror games more exciting or suspenseful. Horror modules are tweaks the GM applies to the rules to make a horror scenario even more scary or to represent how
...
\section{General Horror GM intusions}\index{Horror Rules!Optional Rules!General Horror GM Intrusions}

The following GM intrusions work for most horror genres.

\begin{itemize}
\item Something foils a character’s attempt to escape: a getaway car won’t start, they drop the keys that unlock the exit door or lock up the villain, or the shotgun they’re using to clear a path jams or runs out of shells.
\item The antagonist enters a secure or sealed room by an unexpected method: crashing through a door or wall, crawling out of a ventilation shaft, jumping out of a trap door, manifesting electronically through a Wi-Fi signal, or teleporting.
\item A mysterious noise nearby amplifies the tension, and when investigated reveals itself to be ... a cat, either perfectly calm or hissing and leaping. This often allows for a momentary de-escalation followed by a real scare, such as the antagonist reaching out of the darkness to grab a character.
\item A dramatic and/or ridiculous amount of blood and gore from something that just got killed splashes on a character, blinding them until they take an action to wipe their eyes clean.
\end{itemize}

\section{Bad Penny}\index{Horror Rules!Optional Rules!Bad Penny}
...
\section{Character Posse}\index{Horror Rules!Optional Rules!Character Posse}
...
\section{Dead All Along}\index{Horror Rules!Optional Rules!Dead All Along}
...
\section{Fragility}\index{Horror Rules!Optional Rules!Fragility}
 
...
\section{Ghostly Helpers}\index{Horror Rules!Optional Rules!Ghostly Helpers}
 
...
\section{Hallucination Reset}\index{Horror Rules!Optional Rules!Halucination Reset}
...
\section{Horror Mode}\index{Horror Rules!Optional Rules!Horror Mode}
...
\label{lab:Escalation Rate}

\begin{tabularx}{\textwidth}{| Y | Y |}
\hline

\textbf{Activity} & \textbf{Intrusion Range Increases by 1} \\ \hline

Exploring a large area & Every time a new intrusion is indicated by a die roll \\ \hline
Exploring & Every ten minutes or every time a new intrusion is indicated by a die roll \\ \hline
Combat & Every round \\ \hline
...
\section{Hysteria}\index{Horror Rules!Optional Rules!Histeria}
...
\section{Instant Panic}\index{Horror Rules!Optional Rules!Instant Panic}
...
\section{Ironman}\index{Horror Rules!Optional Rules!Ironman}
...
\section{Last Survivor}\index{Horror Rules!Optional Rules!Last Survivor}
...
\begin{itemize} 
\item All rolls to save them from being killed are eased by two steps.
\item The last survivor XP can be spent only by the last survivor, and only on the last survivor’s rolls, never on any other players’ rolls. (The PC can still spend their personal XP normally, including on other players’ rolls.)
\item At any time, whoever has the token can pass the role of last survivor to another player. The receiving player gets all the XP associated with the last survivor (if there are none, the GM immediately gives 1 XP to the token).
\item Once a player has given up the role of last survivor, they can never again be the last survivor.
\item If the last survivor role has no XP left to spend, and there are no other players to pass the token to (because everyone else has already been the last survivor), the last survivor can pass the token to the GM in exchange for their character getting 1 XP. Once this happens, the last survivor token is removed from the game.
\end{itemize}

\section{Madness}\index{Horror Rules!Optional Rules!Madness}
...
\section{Perilous Venture}\index{Horror Rules!Optional Rules!Perilous Venture}
...
\section{Poor Choices}\index{Horror Rules!Optional Rules!Poor Choices}
...
\subsection{Poor Choices Intrusions}
...
\begin{itemize}
\item Two or more characters sneak off to have sex.
\item A character leaves behind an important piece of equipment, such as a weapon, phone, car keys, or their outer layer of clothes. (The GM can use this intrusion after the fact when a player tries to use a specific item.)
\item A character gets drunk or high.
\item A character falls asleep.
\item A character slips away to urinate out in the woods or a nearby scary building.
\item A character doesn’t care that nearby animals are acting strange (especially if they’re guard dogs).
\item A character doesn’t shoot a dead monster in the head. (“We need to save ammo.”)
\item A character runs away into the dark or away from a place that would be a better, safer direction to run.
\item A character reads aloud words from the weird old book they found, or they play an old recording of someone else reading the book aloud.
\item In a multistory building, a character runs upstairs or down into a basement (where they could get cornered) instead of outside where they could escape in any direction.
\item A character chooses a dumb or obvious hiding place, such as a closet or under a bed.
\item A character tries to escape by squeezing through a space that no human could reasonably get through quickly, such as a doggie door or a tiny window in a garage door.
\item A character hides the fact that they’ve been bitten by a zombie, have a weird rash like the one they saw on the walls of the alien spaceship, or have been hearing a spooky voice telling them to kill their friends. (“I’ll be okay.”)
\item A character runs straight down the road to get away from a pursuing vehicle (instead of onto the sidewalk, behind a big tree, or around a tight corner).
\item A prone or supine character crawls away from approaching danger instead of getting up and running.
\item A character doesn’t call the local authorities for help when they hear something dangerous.
\item A character ignores or rationalizes a weird noise. 
\item A character jumps into the water—a lake, swimming pool, sacred fountain, and so on.
\item A character goes into the cave, mine shaft, or creepy house. (“I’m just going to look around for a second.”)
\item A character insists on staying behind while everyone else goes on ahead. (“Someone should be here when the sheriff shows up!”)
\item A character doesn’t check the back seat of a car before getting in and starting it.
\item A character ignores an obvious creepy clue that there’s something wrong in the house, like a bloody axe, a room full of taxidermy animal heads, or newspaper clippings about recent murders.
\item While being pursued, a character calls for help or otherwise attracts attention (like banging on store windows at midnight).
\item A character tries to pet an unknown lifeform.
\item A character tries to make peaceful contact with an obviously hostile entity. (“It’s as frightened of us as we are of it!”)
\item A character unlocks a door or disables a security system to let a scared stranger into a safe area. 
\item A character doesn’t bother to turn on the lights. 
\item A character uses an action taunting their foe.
\item A character follows a trail of blood.
\item A character ignores good advice from a helpful and knowledgeable NPC. (“That old lady was a superstitious kook.”)
\item A character uses a firearm as a loud, ineffective solution for a simple problem (like shooting a padlock).
\item A character picks up a shady or outright scary-looking hitchhiker.
\item A character scares another character (perhaps by grabbing their shoulder unexpectedly and shouting) as a joke.
\item A character momentarily forgets how to do a simple action, like open or close a door.
\item A character forgets to put their phone on silent mode. 
\item A character imitates or makes fun of a creepy doll or statue.
\item A character tries to help a child who has no reason for being there.
\end{itemize}

\section{Possession}\index{Horror Rules!Optional Rules!Possession}
...
\section{Secret Twist}\index{Horror Rules!Optional Rules!Secret Twist}
...
\section{Shock}\index{Horror Rules!Optional Rules!Shock}
...
\centering
\caption{Shock Levels}
\label{lab:Shock Levels}

\begin{tabularx}{\textwidth}{| Y | p{1.5cm} |}
\hline

\textbf{Event} & \textbf{Level} \\ \hline

Something unexpected darts or jumps out & 1 \\ \hline
Something suddenly moves just out of the corner of the eye & 2 \\ \hline
A sudden loud noise (like a scream) & 2 \\ \hline
Unexpectedly seeing a corpse & 2 \\ \hline
Watching someone die & 3 \\ \hline
Seeing something impossible (like an inanimate object sliding across the floor) & 4 \\ \hline
Watching a friend die & 5 \\ \hline
Seeing a monstrous creature & Creature Level \\ \hline
Witnessing something supernatural (like a spell) & 5 \\ \hline
Seeing something mind-bending (like an impossible, multidimensional demigod coalescing out of thin air) & 8 \\ \hline
...
\section{Unease}\index{Horror Rules!Optional Rules!Unease}
...
\chapter{Consent}\index{Horror!Consent}
