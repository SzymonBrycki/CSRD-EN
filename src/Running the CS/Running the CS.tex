\part{Running the Cypher System}\index{Running the Cypher System}

\begin{multicols}{2}

\chapter{Setting Difficulty Ratings}\index{Running the Cypher System!Setting Difficulty Ratings}
...
\section{The Impossible Difficulties}\index{Running the Cypher System!Setting Difficulty Ratings!The Impossible Difficulties}
...
\section{Routine Actions}\index{Running the Cypher System!Setting Difficulty Ratings!Routine Actions}
...
Player: I \char`_\char`_\char`_\char`_\char`_\char`_\char`_\char`_\char`_.
...
\section{Other Ways to Judge Difficulty}\index{Running the Cypher System!Setting Difficulty Ratings!Other Ways to Judge Difficulty}
...
\section{Advantages to This System}

\begin{enumerate}
\item The GM makes measured adjustments in large, uniform steps. That makes things faster than if players had to do arithmetic using a range of all numbers from 1 to 20.
\item You calculate a target number only once no matter how many times the PCs attempt the action. If you establish that the target number is 12, it’s 12 every time a PC tries that action. (On the other hand, if you had to add numbers to your die roll, you’d have to do it for every attempt.) Consider this fact in light of combat. Once a player knows that they need to roll a 12 or higher to hit a foe, combat moves very quickly.
\item If a PC can reduce the difficulty of an action to 0, no roll is needed. This means that an Olympic gymnast doesn’t roll a die to walk across a balance beam, but the average person does. The task is initially rated the same for both, but the difficulty is reduced for the gymnast. There’s no chance of failure.
\item This is how everything in the game works, whether it’s climbing a wall,
sweet-talking a guard, or fighting a bioengineered horror.
\item Perhaps most important, the system gives GMs the freedom to focus entirely on the flow of the game. The GM doesn’t use dice to determine what happens (unless you want to)—the players do. There aren’t a lot of different rules for different actions, so there is little to remember and very little to reference. The difficulty can be used as a narrative tool, with the challenges always meeting the expected logic of the game. All the GM’s mental space can be devoted to guiding the story.
\end{enumerate}

\chapter{GM Intrusion}\index{Running the Cypher System!GM Intrusion}
...
\section{Using GM Intrusion as a Narrative Tool}
...
\section{Using GM Intrusion as a Resolution Mechanic}
...
\section{Using (and Not Abusing) GM Intrusion} 
...
\section{Intrusion Through Player Rolls}
...
\begin{itemize}
\item In combat, the PC’s foe is not as hurt as they thought. Give the foe 5 extra points of health.
\item In combat, the PC drops their guard, and the foe gets a free attack.
\item In combat, reinforcements for the PC’s foes show up.
\item In combat (or any stressful situation), an ally decides to flee.
\item In combat (or any stressful situation), an ally doesn’t like the PCs as much as they thought. The ally steals from them or betrays them.
\item Out of combat, the PC’s pack falls open, or the sole of their shoe tears open.
\item Out of combat, it begins to rain heavily.
\item Out of combat, a surprise foe appears, and the scene turns into a combat.
\item In an interaction, the GM introduces a surprising motive for the NPC. For example, the PCs are trying to bribe an official for information, and the official reveals that what they really want isn’t money but for someone to rescue their kidnapped son.
\end{itemize}
...
\section{GM Intrusion That Affects the Group}
...
\begin{itemize}
\item The floorboard beneath the PC gives way.
\item The boat lists to starboard at just the wrong moment.
\item A gust of wind blows the papers out of the character’s hand.
\item The buckle of the PC’s pack snaps at an inopportune time.
\item The NPC that the characters need to speak with is home sick today.
\item A device (cypher or artifact) malfunctions or gives the user a jolt.
\end{itemize}

\subsection{An Unknown Complication Emerges}
...
\begin{itemize}
\item A poisonous snake darts out from the tall grass and attacks.
\item The box that holds the plans is trapped with a poison needle.
\item The NPC that the PCs need to befriend doesn’t speak their language.
\item The NPC that the PCs try to bribe is allergic to the bottle of alcohol they offer.
\item The PCs find the book they need, but the pages are so brittle that if they open it, it might crumble.
\end{itemize}

\subsection{An Impending Complication Emerges}
...
\begin{itemize}
\item The goblin reinforcements finally get through the locked door.
\item The ropes of the old rope bridge finally snap.
\item The city guards show up.
\item The unstable ceiling collapses.
...
\subsection{Opponent Luck or Skill}

The PCs aren’t the only ones with surprising tricks up their sleeves. For example:

\begin{itemize}
\item The PC’s opponent uses a lightning-fast maneuver to dodge all attacks.
\item The PC’s opponent sees an opening and makes an additional, immediate attack.
\item The NPC commander rallies their troops, who all deal 2 additional points of damage for one round.
\item The PC’s opponent uses a cypher or similar device that produces just the right effect for the situation.
\item A bit of the wall collapses in the middle of the fight, preventing the characters from chasing the fleeing NPC.
\end{itemize}
...
\begin{itemize}
\item In combat, the PC drops their weapon.
\item In combat, the PC misses and strikes the wall, breaking or damaging their weapon.
\item In combat, the NPC hits the PC harder than usual, inflicting 2 additional points of damage.
\item In combat, the PC hits an ally by accident and inflicts regular damage.
\item Out of combat, the PC drops or mishandles an important object or piece of equipment.
\item In an interaction, the PC inadvertently or unknowingly says something offensive.
\end{itemize}

\subsection{Partial Success}
GM intrusion doesn’t have to mean that a PC has failed. For example:

\begin{itemize}
\item The PC disables the explosive device before it goes off, but if someone doesn’t remain and hold the detonator, it will still explode.
\item The PC creates the antidote, but it will turn the imbiber’s flesh blue for the next few weeks.
\item The PC jumps across the pit but accidentally knocks loose some stones from the edge, making the jump harder for their friend right behind them.
\end{itemize}

\chapter{Player Intrusion}\index{Running the Cypher System!Player Intrusion}
...
\chapter{Tying Actions to Stats}\index{Running the Cypher System!Tying Actions to Stats}
...
\chapter{Cyphers}\index{Running the Cypher System!Cyphers}
...
\begin{itemize}
\item Cyphers should be single-use items. The PCs use them up and find new ones.
\item Cyphers should be potent. A minor ability isn’t worth the trouble. If an attack cypher isn’t as good as a regular weapon, why bother with it?
\item Cyphers shouldn’t have drawbacks.
\item Cyphers should be temporary. Typically, a power is used once. Abilities or advantages that have a duration last from ten minutes to twenty-four hours (at most).
\item Manifest cyphers can take any form. Just make them appropriate to the genre.
\end{itemize}
...
\chapter{Artifacts}\index{Running the Cypher System!Artifacts}
...
\chapter{Skills and Other Abilities}\index{Running the Cypher System!Skills and Other Abilities}
...
\chapter{NPCs and Death}\index{Running the Cypher System!NPCs and Death}
...
\chapter{Creatures}\index{Running the Cypher System!Creatures}
...
\chapter{Balancing Encounters}\index{Running the Cypher System!Balancing Encounters}
...
\section{The challenges the PCs face are routinely too easy or too difficult.}
...
\begin{itemize}
\item Level 1 opponents will be nothing but a nuisance, even in sizable numbers (twelve to sixteen).

\item Level 2 opponents will not be a challenge unless in numbers of twelve or more.

\item Level 3 opponents will be an interesting challenge in numbers of four to eight.

\item Level 4 opponents will be an interesting challenge in numbers of two or three.

\item A single level 5 opponent might be an interesting challenge.

\item A single level 6 opponent will be a serious challenge.

\item A single level 7 or 8 opponent will likely win in a fight.

\item A single level 9 or 10 opponent will win in a fight without breaking a sweat.
\end{itemize}
